...
#ITSA random effects model
library(plm)

NYCLUSchools$ID <- group_indices(NYCLUSchools, NYCLUSchools$School, NYCLUSchools$Exam)

fit4 <- plm(NYCLUSchools$`Percent Passing` ~ NYCLUSchools$Timeelapsed + NYCLUSchools$BTAT + NYCLUSchools$Timeelapsed*NYCLUSchools$BTAT, index=c("ID","Timeelapsed"), data = NYCLUSchools, model = "random")
summary(fit4)
